...
* Pages are edited as a **draft** (up to four variants A–D, each a list of typed blocks). **Publish** validates every
  ...
  image (absolute URL) as `og:*`/`twitter:card` tags; the server renders the same tags without JavaScript, and published
  landing pages without *noindex* are listed in the `landing-pages` sitemap (§ 9).
