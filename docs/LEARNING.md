...
* **Verification page** — `/verify/certificates/{id}`. nginx (`frontend/nginx/default.conf.template`) proxies it to
  `GET /api/v1/public/learning/certificates/{id}/page`, a complete **server-rendered** HTML page (no scripts, own CSP)
  with title/description, canonical, **Open Graph and Twitter tags** (what LinkedIn reads for "Share"), schema.org
  `EducationalOccupationalCredential` JSON-LD, the verification itself and PDF/Share/Open Badge links. The SPA has the
  same route (`features/public/learn/VerifyCertificatePage.tsx`) for in-app navigation and `vite preview`/dev.
  *Hook point for the website SSR work:* if the web tier gains a server-side renderer for public pages, route
  `/verify/certificates/*` to it and reuse `VerificationPage.Render` / the `CertificateVerificationDto` (its `seo` and
  `jsonLd` fields) — or keep the nginx route.
