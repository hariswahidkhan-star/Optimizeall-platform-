...
/// <summary>Looks up a managed redirect for a normalized path. The default implementation has none.</summary>
public interface ISeoRedirectLookup
{
    Task<SeoRedirect?> FindAsync(string path, CancellationToken ct);
}

public sealed class NoSeoRedirects : ISeoRedirectLookup
{
    public Task<SeoRedirect?> FindAsync(string path, CancellationToken ct) => Task.FromResult<SeoRedirect?>(null);
}
    ...
    ISeoRedirectLookup redirects, TimeProvider clock)
{
    ...
    public async Task<SeoPage> ResolveAsync(string path, string? queryString, CancellationToken ct)
    {
        ...
        if (await redirects.FindAsync(path, ct) is { } managed)
        {
            if (managed.Status == 410 || managed.Location is null) return Gone(path);
            return Redirect(path, managed.Location + qs, managed.Status is 301 or 302 or 307 or 308 ? managed.Status : 301);
        }
