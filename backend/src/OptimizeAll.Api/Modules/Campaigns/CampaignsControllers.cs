...
public sealed class CampaignOptionsController(ICampaignAdminService campaigns) : ControllerBase
{
    ...
    // Open to every permission whose pages carry a campaign picker — the review queue (submissions.review), the ledger
    // (ledger.view) and the manager portal (campaigns.manage) — not just campaigns.view, so a custom role granting only
    // one of those pages works too.
