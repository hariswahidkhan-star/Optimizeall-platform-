...
/// <summary>The course packs compiled into the API (<c>Modules/Learning/Catalog/*.json</c>), read and parsed once.</summary>
public static class CoursePackLibrary
{
    private const string Prefix = "OptimizeAll.Learning.Catalog.";
    private static readonly Lazy<IReadOnlyList<PackFile>> Files = new(Load);

    public static IReadOnlyList<PackFile> All => Files.Value;

    private static IReadOnlyList<PackFile> Load()
    {
        var assembly = typeof(CoursePackLibrary).Assembly;
        var result = new List<PackFile>();
        foreach (var name in assembly.GetManifestResourceNames().Where(n => n.StartsWith(Prefix, StringComparison.Ordinal)).OrderBy(n => n, StringComparer.Ordinal))
        {
            using var stream = assembly.GetManifestResourceStream(name)!;
            using var reader = new StreamReader(stream, Encoding.UTF8);
            var json = reader.ReadToEnd();
            var (pack, error) = CoursePack.Parse(json);
            result.Add(new PackFile(name[Prefix.Length..], json, pack, error));
        }
        return result;
    }
}
