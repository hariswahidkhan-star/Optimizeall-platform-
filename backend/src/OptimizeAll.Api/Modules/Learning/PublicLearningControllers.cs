    ...
    CertificateService certificates, PublicLearningService learning, LearningIssuerProvider issuers, AppDbContext db) : ControllerBase
{
    ...
    /// <summary>
    /// The verification page as server-rendered HTML with Open Graph/Twitter tags and JSON-LD. The web server serves it at
    /// <c>/verify/certificates/{id}</c> (frontend/nginx), so crawlers and LinkedIn previews get real metadata.
    /// </summary>
    [HttpGet("certificates/{id:guid}/page")]
    [Produces("text/html")]
    public async Task<IActionResult> Page(Guid id, CancellationToken ct)
    {
        var verification = await certificates.VerifyAsync(id, ct);
        var issuer = await issuers.GetAsync(ct);
        Response.Headers.ContentSecurityPolicy = VerificationPage.ContentSecurityPolicy;
        Response.Headers.CacheControl = "public, max-age=300";
        return Content(VerificationPage.Render(verification, issuer.BaseUrl), "text/html; charset=utf-8", Encoding.UTF8);
    }
