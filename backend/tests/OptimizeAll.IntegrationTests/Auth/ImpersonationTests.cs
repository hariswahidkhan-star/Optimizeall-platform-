...
public sealed class ImpersonationTests(ApiFactory api) : IClassFixture<ApiFactory>
{
    ...
    [Fact]
    public async Task Holders_of_users_impersonate_through_a_custom_role_cannot_be_impersonated()
    {
        var (_, admin) = await AdminAsync();
        var target = await api.CreateUserAsync();
        await api.WithDbAsync(async db =>
        {
            var role = new CustomRole { Name = "Support leads", NormalizedName = CustomRole.Normalize("Support leads " + Guid.NewGuid().ToString("N")[..6]), Permissions = new() { "users.impersonate", "users.view" } };
            db.Add(role);
            db.Add(new UserCustomRole { UserId = target.Id, CustomRoleId = role.Id, AssignedAt = DateTime.UtcNow });
            await db.Set<User>().Where(u => u.Id == target.Id).ExecuteUpdateAsync(s => s.SetProperty(u => u.PermissionVersion, u => u.PermissionVersion + 1));
            await db.SaveChangesAsync();
            return true;
        });

        await (await Impersonating.StartAsync(admin, target.Id)).ShouldFailAsync(403, "admin.impersonation_target_forbidden");
    }
