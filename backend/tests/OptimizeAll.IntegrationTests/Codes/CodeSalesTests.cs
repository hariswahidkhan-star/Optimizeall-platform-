...
public sealed class CodeSalesTests(ApiFactory api) : IClassFixture<ApiFactory>
{
    ...
    [Fact]
    public async Task Code_exports_over_their_cap_are_refused_with_422_and_narrower_filters_export()
    {
        // Two sales (one approved) in one program; caps lowered to 1 on a second host sharing the database.
        var (program, codeId, _, participant, manager) = await kit.AssignedAsync();
        var (_, reviewer) = await kit.ReviewerAsync();
        var a = await kit.SubmitOkAsync(participant, codeId, "CAP-1", net: 100m, discount: 10m);
        await kit.SubmitOkAsync(participant, codeId, "CAP-2", net: 50m, discount: 5m);
        await ApproveAsync(reviewer, Id(a));
        await using var capped = api.WithWebHostBuilder(b => b.ConfigureAppConfiguration((_, c) =>
            c.AddInMemoryCollection(new Dictionary<string, string?> { ["Exports:CodeSales"] = "1", ["Exports:CodeReportSales"] = "1" })));
        await capped.StartAsync();
        var admin = await capped.LoginOnAsync(await api.CreateUserAsync(new[] { Role.Admin }));

        await (await admin.GetAsync($"/api/v1/admin/code-sales/export.csv?programId={Id(program)}")).ShouldFailAsync(422, "export.too_large");
        await (await admin.GetAsync($"/api/v1/admin/code-reports/export.csv?programId={Id(program)}&groupBy=Person"))
            .ShouldFailAsync(422, "export.too_large");
        // Narrowed to one sale, the export has every matching row.
        var approved = await admin.GetAsync($"/api/v1/admin/code-sales/export.csv?programId={Id(program)}&status=Approved");
        approved.EnsureSuccessStatusCode();
        var text = await approved.Content.ReadAsStringAsync();
        Assert.Contains("CAP-1", text);
        Assert.DoesNotContain("CAP-2", text);
    }
