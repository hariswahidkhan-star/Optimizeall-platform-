...
public sealed class CrmBillingPermissionTests(ApiFactory api) : IClassFixture<ApiFactory>
{
    ...
    private static readonly (string Method, string Path, Role[] Allowed)[] Matrix =
    {
        ...
        ("GET", "/api/v1/agency/billing/invoices", new[] { Role.Admin, Role.Finance, Role.AccountManager, Role.SalesRep }),
        ("GET", "/api/v1/agency/billing/reports/aging", new[] { Role.Admin, Role.Finance, Role.AccountManager, Role.SalesRep }),
        ("POST", "/api/v1/agency/billing/invoices", new[] { Role.Admin, Role.Finance }),
        ("POST", $"/api/v1/agency/billing/invoices/{Id}/payments", new[] { Role.Admin, Role.Finance }),
        ("POST", $"/api/v1/agency/billing/invoices/{Id}/void", new[] { Role.Admin, Role.Finance }),
        ("POST", "/api/v1/agency/billing/credit-notes", new[] { Role.Admin, Role.Finance }),
        ("PUT", "/api/v1/agency/billing/settings", new[] { Role.Admin, Role.Finance }),
        ("POST", "/api/v1/agency/billing/tax-rates", new[] { Role.Admin, Role.Finance }),
