...
public sealed class PartnerContentSeedTests(ApiFactory api) : IClassFixture<ApiFactory>
{
    ...
    public async Task Startup_seed_publishes_the_partner_posts_once_and_respects_editor_changes()
    {
        ...
        // Server-rendered (technical SEO): every link to a partner's site in the posts is sponsored and UTM-tagged.
        var partnerLinks = 0;
        foreach (var slug in Slugs)
        {
            var document = await anon.GetAsync("/_document/blog/" + slug);
            Assert.Equal(HttpStatusCode.OK, document.StatusCode);
            var html = await document.Content.ReadAsStringAsync();
            foreach (System.Text.RegularExpressions.Match a in System.Text.RegularExpressions.Regex.Matches(html,
                         "<a [^>]*href=\"https?://([a-z0-9-]+\\.)*(pciai\\.org|certuvo\\.com)[^\"]*\"[^>]*>"))
            {
                partnerLinks++;
                Assert.Contains("rel=\"sponsored noopener\"", a.Value);
                Assert.Contains("utm_source=", a.Value);
            }
        }
        Assert.True(partnerLinks > 0, "the partner posts link to the partners");
