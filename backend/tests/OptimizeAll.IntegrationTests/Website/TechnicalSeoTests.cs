...
public sealed class TechnicalSeoTests(ApiFactory api) : IClassFixture<ApiFactory>
{
    ...
    {
        // Like the real lookup, the Location carries the request's query parameters.
        public Task<SeoRedirect?> FindAsync(string path, string query, CancellationToken ct) => Task.FromResult(path switch
        {
            "/old-seo-page" => new SeoRedirect(301, query.Length == 0 ? "/services/seo" : "/services/seo?" + query),
