...
public sealed class LearningTests(ApiFactory api) : IClassFixture<ApiFactory>
{
    ...
    public async Task Exam_flow_fail_retake_pass_certificate_verification_pdf_badges_and_linkedin()
    {
        ...
        // The verification page is server-rendered with the site's other pages (/_document, docs/SEO_CRO.md § Rendering).
        var page = await anon.GetAsync($"/_document/verify/certificates/{certificateId}");
        Assert.Equal(HttpStatusCode.OK, page.StatusCode);
        Assert.Equal("text/html", page.Content.Headers.ContentType!.MediaType);
        var html = await page.Content.ReadAsStringAsync();
        Assert.Contains("<meta property=\"og:title\"", html);
        Assert.Contains($"<link rel=\"canonical\" href=\"http://app.test/verify/certificates/{certificateId}\"", html);
        Assert.Contains("EducationalOccupationalCredential", html);
        Assert.Contains("Valid certificate", html);
        // Everything is encoded (a holder name is user input).
        await api.WithDbAsync(async db =>
        {
            var cert = await db.Set<Certificate>().SingleAsync(x => x.Id == Guid.Parse(certificateId));
            var original = cert.HolderName;
            cert.HolderName = "Amina <Khan> & Co</script><script>alert(1)</script>";
            await db.SaveChangesAsync();
            var hostile = await (await anon.GetAsync($"/_document/verify/certificates/{certificateId}")).Content.ReadAsStringAsync();
            Assert.DoesNotContain("<script>alert", hostile);
            Assert.DoesNotContain("Amina <Khan>", hostile);
            Assert.Contains("Amina &lt;Khan&gt; &amp; Co", hostile);
            cert.HolderName = original;
            await db.SaveChangesAsync();
        });
        Assert.Equal(HttpStatusCode.NotFound, (await anon.GetAsync($"/_document/verify/certificates/{Guid.NewGuid()}")).StatusCode);
        ...
        var revokedPage = await anon.GetAsync($"/_document/verify/certificates/{certificateId}");
        Assert.Contains("Revoked", await revokedPage.Content.ReadAsStringAsync());
        Assert.Contains("noindex", revokedPage.Headers.GetValues("X-Robots-Tag").Single());
    }
