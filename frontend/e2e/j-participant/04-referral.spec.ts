...
test.describe.serial('referral', () => {
  ...
  test('finance approves the reward; it joins Pat’s approved earnings', async () => {
    ...
    await finance.post(`/finance/pending-earnings/${reward!.id}/approve`, {
      concurrencyStamp: reward!.concurrencyStamp,
    });
    // Approving twice (a retried request) is refused, not applied twice.
    await expect(
      finance.post(`/finance/pending-earnings/${reward!.id}/approve`, {
        concurrencyStamp: reward!.concurrencyStamp,
      }),
