...
async function expectSponsoredOutbound(page: Page) {
  const outbound = page.locator(
    'a[href*="/api/v1/public/partners/"][href*="/visit"], a[href*="pciai.org"], a[href*="certuvo.com"]',
  );
...
test('/partners and the profile pages are indexable, content-rich and mark every partner link as sponsored', async ({
  page,
}) => {
  const errors = watchErrors(page);
  ...
  const visit = await page
    .getByRole('link', { name: /Visit pciai\.org/ })
    .first()
    .getAttribute('href');
  expect(visit).toMatch(
    /^\/api\/v1\/public\/partners\/pci-ai\/visit\?slot=partners\.profile&path=%2Fpartners%2Fpci-ai$/,
  );
...
test('a blog post shows one sponsored unit at its end and the sitemap lists the partner pages', async ({
  page,
}) => {
  const errors = watchErrors(page);
  ...
  // The sitemap index points to the partners sitemap, which lists the partner pages.
  const index = await (await page.request.get('/sitemap.xml')).text();
  expect(index).toMatch(/\/sitemaps\/partners\.xml<\/loc>/);
  const sitemap = await (await page.request.get('/sitemaps/partners.xml')).text();
