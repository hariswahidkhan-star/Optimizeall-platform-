        ...
        <div className="lx-video__soon">
          ...
          <div>
            ...
            <p className="lx-muted">
              This lesson’s video is in production. Everything it covers is in the article below.
            </p>
          </div>
        </div>
      )}
...
export function KnowledgeCheckQuestion({
  ...
  const toggle = (i: number) => {
    setResult(null);
    setSelected(
      check.multiple ? (selected.includes(i) ? selected.filter((x) => x !== i) : [...selected, i]) : [i],
    );
  };
        ...
        {check.options.map((option, i) => {
          ...
          return (
            <label
              key={i}
              htmlFor={id}
              className={clsx('lx-option', selected.includes(i) && 'is-selected', state && `is-${state}`)}
            >
              <input
                id={id}
