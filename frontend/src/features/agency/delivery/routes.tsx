import { Building2, Clock, FileBarChart, FileCheck2, FolderKanban, LayoutDashboard, LayoutTemplate, ListTodo } from 'lucide-react';
...
import { DeliverableReviewPage, DeliverablesPage } from './DeliverablesPage';
import { MyTasksPage } from './MyTasksPage';
import { ProjectDetailPage } from './ProjectDetailPage';
import { ProjectsPage } from './ProjectsPage';
import { ReportBuilderPage, ReportsPage } from './ReportsPages';
...
export const nav: PortalNavItem[] = [
  ...
  { to: 'clients', label: 'Clients', icon: Building2, description: 'Client accounts, teams, onboarding and health.', requires: requires.clients },
  { to: 'projects', label: 'Projects', icon: FolderKanban, description: 'Retainers, campaigns and builds with kanban boards.', requires: requires.projects },
  { to: 'tasks', label: 'My tasks', icon: ListTodo, description: 'Everything assigned to you.', requires: requires.projects },
  { to: 'deliverables', label: 'Deliverables', icon: FileCheck2, description: 'Internal review and client approvals.', requires: requires.projects },
  { to: 'time', label: 'Time', icon: Clock, description: 'Timer, timesheets and utilization.', requires: requires.time },
  { to: 'reports', label: 'Reports', icon: FileBarChart, description: 'Monthly client performance reports.', requires: requires.reports },
  { to: 'templates', label: 'Templates', icon: LayoutTemplate, description: 'Service and brief templates.', requires: requires.projects },
];
