...
import { NETWORK_LABELS, type CalendarResponse, type Media, type Post, type PostSummary, type SocialKpis, type SeriesPoint, type TopPost } from '@/features/agency/social/api';
...
interface AdsSummary {
  ...
  totals: { spend: number; impressions: number; clicks: number; conversions: number; conversionValue: number };
  kpis: { cpa: number | null; roas: number | null; ctr: number | null };
  platforms: { platform: string; totals: { spend: number; conversions: number }; kpis: { roas: number | null; cpa: number | null }; sourceLabel: string }[];
  fxMissing: string[];
  sourceLabel: string;
}
...
const clientKeys = {
  ...
  performance: (org: string, from: string, to: string) => ['client-social', 'performance', org, from, to] as const,
};

/** The organization the client user is looking at (first one by default; switchable when they belong to several). */
function useOrganization(): { org: Organization | undefined; picker: ReactElement | null; loading: boolean; error: unknown } {
  const orgs = useQuery({ queryKey: clientKeys.orgs, queryFn: () => api.get<Organization[]>('/client/social/organizations') });
  ...
  const picker =
    list.length > 1 ? (
      <FormField label="Organization">
        <Select value={org?.id ?? ''} onChange={(e) => setSelected(e.target.value)} options={list.map((o) => ({ value: o.id, label: o.name }))} />
      </FormField>
    ) : null;
  return { org, picker, loading: orgs.isLoading, error: orgs.error };
}

function Gate({ state, children }: { state: ReturnType<typeof useOrganization>; children: (org: Organization) => ReactElement }) {
  if (state.loading) return <Skeleton height="16rem" />;
  if (state.error) return <ErrorState error={state.error} />;
  if (!state.org) return <EmptyState icon={<CalendarDays />} title="No organization" description="Your account is not linked to a client organization yet." />;
...
export function ClientSocialCalendarPage() {
  ...
  return (
    <>
      <PageHeader title="Social calendar" description="Posts planned, awaiting your approval, scheduled and published." />
...
function CalendarList({ org, month }: { org: Organization; month: string }) {
  ...
  const query = useQuery({
    ...
    queryFn: () => api.get<CalendarResponse>('/client/social/calendar', { query: { clientId: org.id, from, to } }),
  });
  const columns: DataTableColumn<PostSummary>[] = [
    { id: 'when', header: 'When', primary: true, cell: (p) => (p.scheduledAt ? <DateTime value={p.scheduledAt} format="datetime" /> : '—') },
    { id: 'title', header: 'Post', cell: (p) => p.title },
    { id: 'preview', header: 'Text', hideOnMobile: true, cell: (p) => <span className="sm-muted">{p.preview}</span> },
    {
      id: 'networks',
      header: 'Networks',
      cell: (p) => (
  ...
  return (
    <div className="stack">
      <DataTable
        ...
        emptyState={<EmptyState compact icon={<CalendarDays />} headingLevel={2} title="Nothing planned this month" />}
      />
...
export function ClientApprovalsPage() {
  ...
  return (
    <>
      <PageHeader title="Approvals" description="Review posts before they are scheduled. Approve them, or request changes with a note for the team." />
    ...
    <div className="stack">
      {!org.canApprove && (
        <Alert tone="info" title="View only">
          Your role ({org.role}) can see posts but not approve them. Ask an Approver or Owner in your organization.
        </Alert>
      )}
      {posts.length === 0 ? (
        <EmptyState icon={<CheckCircle2 />} title="You're all caught up" description="No posts are waiting for your approval." />
      ) : (
        <ul className="stack" style={{ listStyle: 'none', padding: 0, margin: 0 }} aria-label="Posts awaiting approval">
          {posts.map((p) => (
            <li key={p.id}>
              <Card as="article" aria-labelledby={`appr-${p.id}`}>
                <CardHeader
                  title={p.title}
                  titleId={`appr-${p.id}`}
                  description={p.scheduledAt ? `Planned for ${new Date(p.scheduledAt).toLocaleString()}` : undefined}
                  actions={
...
function ReviewDialog({ postId, org, onClose }: { postId: string; org: Organization; onClose: () => void }) {
  ...
  const post = useQuery({ queryKey: clientKeys.post(postId), queryFn: () => api.get<Post>(`/client/social/posts/${postId}`) });
  const media = useQuery({ queryKey: ['client-social', 'media', postId], queryFn: () => api.get<Media[]>('/client/social/media', { query: { postId } }) });
  ...
  const decide = useMutation({
    mutationFn: (kind: 'approve' | 'request-changes') =>
      api.post<Post>(`/client/social/posts/${postId}/${kind}`, { comment: comment || undefined, concurrencyStamp: post.data?.concurrencyStamp }),
    onSuccess: (_, kind) => {
    ...
    <Dialog
      ...
      footer={
        org.canApprove && p?.allowedActions.includes('clientApprove') ? (
          <>
            <Button variant="secondary" leadingIcon={<MessageSquareWarning />} disabled={!comment.trim()} onClick={() => decide.mutate('request-changes')}>
              Request changes
            </Button>
            <Button variant="highlight" leadingIcon={<CheckCircle2 />} loading={decide.isPending} onClick={() => decide.mutate('approve')}>
              Approve
            </Button>
          </>
        ) : (
      ...
      {!p ? (
        ...
        <div className="stack">
          ...
          {p.variants.length > 1 && (
            <FormField label="Network">
              <Select
                ...
                options={p.variants.map((x, i) => ({ value: String(i), label: `${NETWORK_LABELS[x.network]} @${x.profileHandle}` }))}
              />
            </FormField>
          )}
          {v && (
          ...
          {p.comments.length > 0 && (
            <Timeline
              label="Comments"
              items={p.comments.map((c) => ({ id: c.id, title: c.authorName, description: c.body, timestamp: c.createdAt }))}
            />
          )}
          {org.canApprove && (
...
export function ClientPerformancePage() {
  ...
  return (
    <>
      <PageHeader title="Social & ads performance" description="Results across your social profiles and ad accounts. Each figure shows whether it was measured or entered manually." />
      <div className="sm-toolbar">
        {state.picker}
...
function PerformanceBody({ org, days }: { org: Organization; days: number }) {
  ...
  const query = useQuery({
    ...
    queryFn: () => api.get<Performance>('/client/social/performance', { query: { clientId: org.id, from, to } }),
  });
  if (query.isError) return <ErrorState error={query.error} />;
    ...
    <div className="stack">
      <section aria-labelledby="perf-social" className="stack">
        ...
        <div className="sm-grid-stats">
          ...
          <Stat label="Followers growth" value={social.totals.followersGrowth == null ? '—' : formatNumber(social.totals.followersGrowth, { signDisplay: 'exceptZero' })} measurement={m} />
        ...
        {topPosts.length > 0 && (
          <DataTable
            caption="Top posts"
            columns={[
              { id: 'post', header: 'Post', primary: true, cell: (p: TopPost) => <span><NetworkChip network={p.network} /> {p.title ?? 'Post'}</span> },
              { id: 'eng', header: 'Engagements', align: 'right', cell: (p: TopPost) => formatNumber(p.engagements) },
              { id: 'er', header: 'Eng. rate', align: 'right', cell: (p: TopPost) => percent(p.engagementRate) },
              { id: 'link', header: 'Link', cell: (p: TopPost) => (p.url ? <SafeExternalLink href={p.url}>View</SafeExternalLink> : '—') },
            ]}
            rows={topPosts}
      ...
      <section aria-labelledby="perf-ads" className="stack">
        ...
        {ads.fxMissing.length > 0 && <Alert tone="warning">Some spend could not be converted to {cur} yet ({ads.fxMissing.join(', ')}).</Alert>}
        <div className="sm-grid-stats">
          <Stat label="Ad spend" value={formatMoney(ads.totals.spend, cur)} measurement="Measured" />
          <Stat label="Conversions" value={formatNumber(ads.totals.conversions, { maximumFractionDigits: 1 })} measurement="Measured" />
          <Stat label="Cost per acquisition" value={ads.kpis.cpa == null ? '—' : formatMoney(ads.kpis.cpa, cur)} measurement="Measured" />
          <Stat label="Return on ad spend" value={ads.kpis.roas == null ? '—' : `${ads.kpis.roas.toFixed(2)}×`} measurement="Measured" />
        </div>
        {ads.platforms.length > 0 && (
          <ul className="cluster" style={{ listStyle: 'none', padding: 0 }}>
            {ads.platforms.map((p) => (
              <li key={p.platform}>
                <Badge tone="brand">
                  {p.platform}: {formatMoney(p.totals.spend, cur)} · ROAS {p.kpis.roas == null ? '—' : p.kpis.roas.toFixed(2)}
                </Badge>
              </li>
            ))}
          </ul>
