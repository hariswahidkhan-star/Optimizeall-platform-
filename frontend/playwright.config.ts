...
/** Social media + paid ads journey: serial on one worker, desktop only (agency tools and the client portal on desktop). */
const socialJourney = suite === 'j-social';
/** Full-stack suites share one database and build on earlier steps: serial, one worker, no retries. */
const journeys = suite === 'journeys' || responsiveSplit || campaignJourney || socialJourney;
...
export default defineConfig({
  ...
  projects: [
    ...
    ...(a11y || crawl || campaignJourney || socialJourney
      ? []
      : [
          {
